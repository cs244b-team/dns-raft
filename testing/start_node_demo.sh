...
"sudo rm -rf /tmp/*.logents /tmp/*.state /tmp/*.log && cd dns-raft/cmd/server && go build . && sudo -b LOG_LEVEL=INFO ./server --node ${INTERNAL_IPS[0]}:9000 --node ${INTERNAL_IPS[1]}:9000 --node ${INTERNAL_IPS[2]}:9000 --node ${INTERNAL_IPS[3]}:9000 --node ${INTERNAL_IPS[4]}:9000 -id ${1} --port 53 > /tmp/node-${1}.log 2>&1"
